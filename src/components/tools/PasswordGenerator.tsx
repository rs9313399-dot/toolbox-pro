...
export default function PasswordGenerator({ onNavigate }: PasswordGeneratorProps) {
  ...
  return (
    <ToolLayout
      ...
      seoContent={`
        <h2>Secure Password Generator — Create Strong Passwords Instantly</h2>
        <p>In an era of increasing cyber threats, using strong and unique passwords for every account is one of the most important steps you can take to protect your digital life. Our free online password generator creates cryptographically secure passwords right in your browser, ensuring that your credentials are never transmitted over the internet. Whether you need a simple 8-character code or a complex 128-character passphrase, this tool gives you full control over length and character composition.</p>
        <h3>Why Strong Passwords Matter</h3>
        <p>According to cybersecurity research, over 80% of data breaches are caused by weak or reused passwords. Attackers use sophisticated brute-force tools that can guess millions of password combinations per second. A password like "123456" or "password" can be cracked in less than a second, while a randomly generated 16-character password with uppercase, lowercase, numbers, and symbols would take billions of years to crack with current technology. Using unique passwords for each account also prevents credential stuffing attacks, where a breach on one site exposes your accounts everywhere.</p>
        <h3>How Our Password Generator Works</h3>
        <p>This tool uses the Web Crypto API's <code>crypto.getRandomValues()</code> method, which provides cryptographically secure random numbers generated by your operating system. Unlike <code>Math.random()</code>, which is predictable and not suitable for security purposes, <code>crypto.getRandomValues()</code> produces truly unpredictable values. The generator works entirely client-side — no data is ever sent to a server, and your generated passwords exist only in your browser's memory.</p>
        <h3>Key Features</h3>
        <ul>
          <li>Configurable password length from 8 to 128 characters</li>
          <li>Toggle uppercase letters (A–Z), lowercase letters (a–z), numbers (0–9), and special symbols (!@#$%^&amp;*)</li>
          <li>Real-time password strength indicator based on entropy calculation</li>
          <li>One-click copy to clipboard for instant use</li>
          <li>Completely offline — no server communication, maximum privacy</li>
        </ul>
        <h3>Tips for Password Management</h3>
        <ul>
          <li>Use a different password for every account — never reuse passwords across services</li>
          <li>Pair generated passwords with a reputable password manager like Bitwarden, 1Password, or KeePass</li>
          <li>Enable two-factor authentication (2FA) wherever possible for an additional layer of security</li>
          <li>Aim for at least 16 characters for high-value accounts such as banking and primary email</li>
          <li>Change passwords immediately if you suspect a breach — use Have I Been Pwned to check</li>
        </ul>
      `}
    >
